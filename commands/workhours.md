...
- 非今天的记录 → 该天上午和下午都「已完成」
- 今天的记录：
  - 当前时间 ≥ 12:00 → 今天上午「已完成」，今天下午「未完成」
  - 当前时间 < 12:00 → 今天上午「未完成」，今天下午「未完成」

**第二步：对每天每个时段分配工时**

对每个「已完成」的时段（上午或下午，固定 4h）：
1. 统计该时段所有有效提交，区分出「有 `-t`」和「无 `-t`」两组
2. 计算有 `-t` 的记录总时长（如 1.5h + 1h = 2.5h）
3. 剩余时间 = 4h - 有 `-t` 总时长（最小为 0）
4. 剩余时间 ÷ 无 `-t` 的条数 = 每条自动分配时长（保留 1 位小数，最小 0.5h）

对「未完成」的时段：
- 有 `-t` 的记录 → 正常显示指定时长
- 无 `-t` 的记录 → **跳过，不计入工时，不展示**（时段未结束，无法准确分配）

**第三步：按项目分组，累加工时**

将所有展示的记录按 `project` 分组，累加各记录工时得到项目总工时。

举例（今天下午2点导出本周）：
- 周一上午 3 条（1 条有 `-t 1h`，2 条无 `-t`）→ 剩余 3h ÷ 2 = 每条 1.5h ✅
- 今天上午 2 条（`其他` 有 `-t 1.5h`，`workhours-claude` 无 `-t`）→ 剩余 4-1.5=2.5h ÷ 1 = 2.5h ✅
- 今天下午 1 条无 `-t` → **不显示，跳过** ❌（下午未结束）
- 今天下午 1 条有 `-t 2h` → 正常显示 2h ✅
